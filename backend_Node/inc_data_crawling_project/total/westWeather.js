...
exports.westWeather = () => {
    ...
    const GetData = async(csrf, cookie) => {
        return new Promise((resolve, reject) => {
            ...
            req.write(bodyData);
            req.on('error', (err) => logger.error(`westWeather ${err}`));
            req.on('timeout', (timout) => {logger.error(`westWeather 타임아웃 ${timout}`);});
            req.end();
        })
    }

    const westWeatherValue = async(keyData, valueData, dataDate) => {
        return new Promise((resolve, reject) => {
            let InsertValue = [];
                                                                ...
                                                                valueData[i] === '서남서' ? 247.5 : (
                                                                    valueData[i] === '서' ? 270 : (
                                                                        valueData[i] === '서북서' ? 292.5 : (
                                                                            valueData[i] === '북서' ? 315 : (
                                                                                valueData[i] === '북북서' ? 337.5 : (
                                                                                    valueData[i] === '동북동' ? 67.5 : valueData[i]
                                                                                ) 
                                                                            )
                                                                        )
                                                                    )
        ...
        return new Promise(async(resolve, reject) => {
            for (let i = 0; i < data.length; i++) {
                await fetch(`http://${api.apiUrl}/api/crawling/raw`, {
                    ...
                    logger.error(`westWeather.js_125 : ${error}`);
                    resolve();
                });
            }
        })
    ...
    const run = async () => {
        ...
        logger.info(`westWeather cookie&csrf success`);
        ...
        if(jsonData.data.length !== 0){
            ...
            session = await westWeatherValue(KeyData, ValueData, dataDate);
            logger.info(`westWeather data posting`);
            session = await weatherPost(session);
            logger.info(`westWeather 데이터 전송 성공`); 
        }else{
            logger.info(`westWeather 데이터 0개`);
        }
    }

    run().catch((reason) => {
        logger.error(`westWeather.js : ${reason}`);
        return null;
    });
