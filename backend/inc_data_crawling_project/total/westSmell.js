        ...
        try{
            fetch('http://119.197.244.110:8080/getReportList', {
                ...
                headers: {
                    ...
                    "Referer": "http://119.197.244.110:8080/odor/list?cate=1526870188221&subcate=1526870201953",
                    ...
                    for (let i = 0; i < InsertValue.length; i++) {
                        await new Promise((resolve, reject) => {
                            fetch(`http://${api.apiUrl}/api/crawling/raw`, {
                                ...
                                logger.error(`westSmell.js_93 : ${error}`);
                                resolve();
                            })
                        })
                    }
            ...
            logger.error(`westSmell.js_101 : ${err}`);
        }
    })
    req.on('timeout', () => {
        logger.error(`westSmell.js_103 : 서버 타임 아웃`);
        req.abort();
    });
    req.on('error', (error) => {
        logger.error(`westSmell.js_108 : ${error}`);
    });
    req.end(); 
    logger.info(`westSmell 악취 success`);
}
