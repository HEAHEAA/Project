...
function Station03HTBC({page, rowsPerPage, dvList}) {
    return (
        <div>
                ...
                <Table sx={{minWidth: "100%", fontSize: 11}} aria-label="simple table" className="cw02Table">
                    <TableHead style={TableHeader}>
                        <TableRow>
                            ...
                            <TableCell size={"small"} align={"center"} style={TextWhite}>운전기간(시작일)</TableCell>
                            <TableCell size={"small"} align={"center"} style={TextWhite}>운전기간(종료일)</TableCell>
                            <TableCell size={"small"} align={"center"} style={TextWhite}>운전시간(시작일)</TableCell>
                            <TableCell size={"small"} align={"center"} style={TextWhite}>운전시간(종료일)</TableCell>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {
                                ...
                                ?.map(function (arr, inx) {
                                    return (
                                        <TableRow key={(rowsPerPage * page) + inx} className="tableHover">
                                            ...
                                            <TableCell
                                                size={"small"} style={TableCells}> {arr.oprtStMd?.replace(/(\d)(?=(?:\d{2})+(?!\d))/g, '$1:')} </TableCell>
                                            <TableCell
                                                size={"small"} style={TableCells}> {arr.oprtEdMd?.replace(/(\d)(?=(?:\d{2})+(?!\d))/g, '$1:')} </TableCell>
                                            <TableCell
