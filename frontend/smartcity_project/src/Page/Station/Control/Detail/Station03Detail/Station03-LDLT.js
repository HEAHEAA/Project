...
function Station03LDLT({page, rowsPerPage, dvList}) {
    return (
        <div>
                                ...
                                ?.map(function (arr, inx) {
                                    return (
                                        <TableRow key={(rowsPerPage * page) + inx} className="tableHover">
                                            ...
                                            <TableCell size={"small"} style={TableCells}> {arr.oprtStHm?.replace(/(\d)(?=(?:\d{2})+(?!\d))/g, '$1:')} </TableCell>
                                            <TableCell size={"small"} style={TableCells}> {arr.oprtEdHm?.replace(/(\d)(?=(?:\d{2})+(?!\d))/g, '$1:')} </TableCell>
                                        </TableRow>
                                    )
                                })
                        }
